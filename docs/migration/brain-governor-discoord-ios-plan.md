...
| 2026-09-02 | 6 | Added desktop open-list navigation for the main PWA | Main-profile desktop layout now keeps the mobile dropdown hidden on wide screens, renders the personal sections as an always-open Agenda/Calendar/Tasks/Supplies/Memos/Documents/Fax/Mail/Utils/Settings list in the squared left menu card, and pins the circular page-aware `+` in the card header area; versioned `styles.css?v=272` and `app.js?v=263`; JavaScript syntax check, 31 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; mobile/PWA dropdown behavior remains unchanged |
