...
| 2026-09-02 | 6 | Anchored the desktop shell plus to the left card | Commit `6509077`; desktop main menu picker no longer acts as the positioning parent for the shell `+`, so the button anchors to the left card/header instead of drifting beside the active menu row; versioned `styles.css?v=278`; JavaScript syntax check, 35 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and H3 served-asset smoke pass | H3 static portal promoted; CSS-only desktop visual fix with no action/API behavior change; H4 sync pending |
