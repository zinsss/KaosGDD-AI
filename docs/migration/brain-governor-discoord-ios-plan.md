...
| 2026-09-02 | 6 | Show readable Paperless tags in Documents detail and editor | Paperless document detail now resolves tag IDs to names when possible, keeps tag IDs if tag lookup is unavailable, includes readable `tags` in the Governor/PWA document contract, displays tags in Documents Archive detail metadata, and prefills the Archive metadata editor with `#tag` names; versioned `documents.js?v=5` and `app.js?v=262`; JavaScript syntax check, 30 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and 369 containerized Governor/Discoord/Brain tests pass | Deployment pending; Archive rows remain uncluttered as number/date/title only, while tags live in detail/edit panels |
