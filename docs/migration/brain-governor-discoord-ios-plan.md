...
| 2026-09-02 | 6 | Restored Supplies through a governed PWA API | Personal Supplies no longer calls the retired legacy `/api/supplies` route; `family-portal` now routes `/api/supplies` to `governor-api`, Governor maps list/create/done/active/delete to the existing Radicale supplies `VTODO` profile, and the PWA Supplies page renders as an Archive Terminal board with `#/add-supply` opened by the shell `+`; versioned `navigation.js?v=4` and `app.js?v=265`; JavaScript syntax check, 39 frontend tests, Governor API syntax check, deploy-helper syntax check, compose validation, `git diff --check`, and 369 containerized H3 tests pass | Deployment pending; expected production impact is restoring personal Supplies list/add/done/delete via Governor while keeping retired `/embed/agenda-supplies` closed |
