...
| 2026-09-02 | 6 | Repositioned the desktop shell plus button | Commit `f391859`; desktop main PWA left-card `+` now anchors in the KaosGDD header band instead of floating beside the active menu row, with the menu list starting lower for breathing room; versioned `styles.css?v=276`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and H3 served-asset smoke pass | H3 static portal promoted; CSS-only desktop visual polish with no action/API behavior change; H4 sync pending |
