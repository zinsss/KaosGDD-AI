...
| 2026-09-04 | 7 | Added treatment-option source routing for AI Tasks web search | Prompt-only AI Tasks can now be classified as `treatment_options`, causing KaosBrain to request source-bounded treatment/guideline summaries and Governor to prioritize trusted treatment sources such as 국가건강정보포털, 국가정신건강정보포털, PubMed, NIH/NINDS, NICE CKS, and AASM while keeping `health.kr` enrichment limited to medicine/drug-benefit lookups. Live probe for `하지불안증후군 치료 옵션` ranks current AASM/PubMed guideline results above older/noisy PubMed matches. The architecture doc also records the later indexed PDF textbook source tier: server-side PDFs should be extracted/OCRed once, chunked with page/section/hash metadata, locally indexed, and used only as cited background after current official/guideline sources | Validation passes: Python syntax checks, frontend tests (`78/78`), `git diff --check`, H3 helper tests (`369 OK`), and Brain helper tests (`358 OK`). Deployment pending in this turn. Production impact is read-only AI Task search/result quality only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes |
