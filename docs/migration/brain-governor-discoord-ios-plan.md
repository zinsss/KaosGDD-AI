...
| 2026-09-02 | 6 | Aligned Documents upload and metadata form rows | Documents upload `FILE`/`TITLE` rows and detail metadata editor `TITLE`/`TAGS` rows now share a fixed label column so their controls align cleanly in the archive-terminal UI; versioned `styles.css?v=273`; JavaScript syntax check, 32 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; CSS-only visual polish with no upload, Paperless metadata, or Governor behavior change |
