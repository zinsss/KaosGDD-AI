...
| 2026-09-02 | 6 | Corrected personal portal default branding | Commit `85315f3`; `kaosgdd.net` startup/default HTML now identifies as `KaosGDD` instead of `Kaos Family`, including the page title, iOS web-app title, and initial visible fallback header; the `family.kaosgdd.net` host override still switches to `Kaos Family`; JavaScript syntax check, 35 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and H3 served-HTML smoke pass | H3 static portal promoted; HTML/test-only branding correction with no route/API behavior change; H4 sync pending |
