...
| 2026-09-04 | 7 | Added 약학정보원 medicine dictionary enrichment for AI Tasks search | Commit `pending`; Governor now treats `health.kr` as a read-only medicine dictionary helper for product-name, ingredient-name, MFDS classification, and category hints. For medicine benefit queries, Governor can first search health.kr product JSON with CSRF/session handling, fetch the selected drug-detail JSON, derive English/Korean ingredient and category terms, and then search HIRA 보험인정기준 with those enriched queries. Focused validation proves an unknown brand can be expanded through health.kr into an ingredient before HIRA search, and live smokes return HIRA criteria for both `알모그란정 급여기준` and `종근당글리아티린 요양급여기준` (`choline alfoscerate 경구·시럽·주사제` first). Syntax checks and H3 Docker suite (`369 tests OK`) pass | Deployment pending at the time of this log entry. `health.kr` is not treated as the final authority for insurance-benefit decisions; if HIRA criteria are found for a HIRA-preferred query, only HIRA candidates are returned to the summarizer |
