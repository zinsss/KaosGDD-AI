...
| 2026-09-03 | 7 | Added AI-assisted Paperless tag suggestions for Documents review | Personal Documents metadata review now has an `AI TAGS` action that asks Governor for Paperless document context, forwards title/OCR/current-tags plus the existing Paperless tag list to KaosBrain, and fills the editable tag field with matching existing tags only; manual tags are preserved, metadata `PREVIEW` and confirmed `APPLY` remain the only Paperless write path, and `app.js?v=288`; focused portal checks, H3 container tests (`369 tests OK`), and Brain image tests (`342 tests OK`) pass | H3/H4 deployment pending in this turn. Future tuning: improve the Paperless tag vocabulary and add archive-detail metadata editing if real review shows missing categories |
