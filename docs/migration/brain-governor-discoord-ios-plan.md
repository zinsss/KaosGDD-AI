...
- Phase 8 slice 2 is deployed in commit `997c5e8`: the personal PWA Settings
  page exposes the same read-only system status through Governor, with a
...
- Phase 6 Fax attention ACK deployed on 2026-09-02 in commit `f386471`: the
  PWA Fax marker changed from historical failed-count behavior to
  unacknowledged-failure behavior. Failed records stay in the archive, but a
  failed fax detail can record an ACK marker so the main menu red dot stops
  until a changed/new failure appears. H3 served `fax.js?v=2` and `app.js?v=272`
  after `governor-api` was recreated with writable fax state.
...
2. Add read-only `system.status` to Brain Guard and the personal admin PWA.
   ...
   write controls. A navigation-only link to Discord `#brain` is allowed.
...
Stored maintenance read boundary (2026-09-02):

- documented the planning-boundary security review and approved only a local
  parser for the existing fixed maintenance-report path;
- caps input at 256 KiB, selects exactly one `kaosgdd` target, enforces a
  timezone-aware maximum 48-hour age, and normalizes cached package, Docker
  Engine package, reboot, and container-health counts;
- rejects missing, malformed, failed, duplicate, mismatched, future-dated,
  stale, and invalid-count reports without returning arbitrary report text;
...
| 2026-09-01 | 6 | Replaced personal Memos iframe with a native archive board | Personal Memos now renders through the main PWA using the shared Archive Terminal UI, reads list/search data from the existing Governor Memos relay, shows memo rows as number/date/title only, and keeps memo details local to the loaded list response without widening backend relay permissions; versioned `styles.css?v=266` and `app.js?v=246`; JavaScript syntax check, 18 frontend tests, `git diff --check`, nginx validation, and served-file byte sync pass | H3 static portal promoted; Memos backend/image/data unchanged |
| 2026-09-01 | 6 | Deployed first Documents upload workflow | Commit `5f82adb` plus tracker commit `f8bf5b8`; added a governed PDF upload path from the personal PWA to Paperless, backed by a small `/data/documents/intake.json` Inbox board with SHA duplicate protection; `#/add-document` is owned by Documents and the top `+` opens it; versioned `styles.css?v=267`, `documents.js?v=3`, and `app.js?v=247`; 19 frontend tests and 267 Governor tests pass; H3 `governor-api` is healthy, static files are byte-synced, and H4 Brain doctor passes at `f8bf5b8` | Deployed to H3 and synced to H4; no synthetic PDF uploaded to live Paperless during validation |
| 2026-09-01 | 5 | Added Naver lifecycle polling for INBOX | Commit `f4b2cb2`; changed the Naver mail lifecycle default and H3 runtime configuration to `MAIL_NAVER_FOLDERS=INBOX,세무사,영덕군보건소`; 22 focused mail/worker tests pass; H3 worker/Discord smoke passes; live Naver state now has 3 checkpointed mailboxes (`INBOX`, `세무사`, `영덕군보건소`) with `INBOX` baselined at UID 49980, no pending items, and no mail error | Deployed to H3; rollback state snapshot `naver-discord.json.pre-inbox-20260901-155442` and worker image `rollback-pre-mail-inbox-20260901` retained |
| 2026-09-01 | 6 | Deployed read-only personal PWA Mail board | Commit `c01f55f`; added protected Governor `GET /api/mail/messages` around the existing read-only Naver header poller plus a native Archive Terminal Mail board in the personal PWA; `governor-api` receives Naver config/secret and mounts `/data/mail` read-only; versioned `mail.js?v=1`, `styles.css?v=268`, and `app.js?v=248`; 21 frontend tests and 272 Governor tests pass; live read-only smoke saw 3 mailboxes and 1 returned header for `limit=1`; unauthenticated API returns 401 | Deployed to H3; rollback image `kaosgdd-ai-governor-api:rollback-pre-mail-board-20260901` retained; no mail read/move/archive/import mutation occurred |
| 2026-09-01 | 6 | Deployed read-only PWA Mail detail body and attachment metadata | Commit `16a2be1`; added protected Governor `GET /api/mail/messages/{uid}?mailbox=...` using read-only IMAP `BODY.PEEK[]`, plus PWA detail loading/error states and attachment metadata display; versioned `mail.js?v=2` and `app.js?v=249`; 22 frontend tests, 277 Governor tests, served-file byte sync, unauthenticated 401, and live detail smoke pass | Deployed to H3; rollback image `kaosgdd-ai-governor-api:rollback-pre-mail-detail-20260901` retained; H4 synced and KaosBrain doctor passes |
| 2026-09-01 | 6 | Deployed read-only PWA Mail attachment viewing | Commit `70b87a6`; added protected Governor `GET /api/mail/messages/{uid}/attachments/{index}?mailbox=...` to stream a single Naver attachment inline with `BODY.PEEK[]`, added per-attachment open links in the Mail detail panel, and versioned `styles.css?v=269`, `mail.js?v=3`, and `app.js?v=250`; 22 frontend tests, 280 Governor tests, served-file byte sync, unauthenticated 401, and live attachment smoke pass | Deployed to H3; rollback image `kaosgdd-ai-governor-api:rollback-pre-mail-attachments-20260901` retained; attachment viewing does not create Documents Inbox or Paperless items; H4 synced and KaosBrain doctor passes |
| 2026-09-01 | 6 | Fixed personal PWA Mail same-origin API routing | Added the missing `family-portal` nginx `location ^~ /api/mail/` proxy to `governor-api:8096` and a deploy preflight assertion, resolving the browser-side `HTTP 200` error caused by static `index.html` being returned for Mail API calls; container-local smoke with `Host: kaosgdd.net` now returns Governor 401 for unauthenticated list and attachment routes instead of static 200 | H3 static portal nginx promoted; H4 synced and KaosBrain doctor passes |
| 2026-09-01 | 6 | Scoped personal PWA Mail board to operational folders | Commit `6f36d94`; added a read-only Governor `folder=` query scope for `GET /api/mail/messages`, rejected unconfigured requested roots, and changed the personal PWA Mail board to request only `영덕군보건소` and `세무사`; `INBOX` remains configured for lifecycle polling/notifications but is not fetched by the PWA; versioned `app.js?v=251`; 22 frontend tests and 283 Governor tests pass; live filtered smoke returned 2 mailboxes, 20 messages, and `containsInbox: False` | Deployed to H3; rollback image `kaosgdd-ai-governor-api:rollback-pre-mail-folder-scope-20260901` retained; H4 synced and KaosBrain doctor passes |
| 2026-09-01 | 6 | Added personal PWA Mail board tabs | Commit `b97052f`; added client-side Mail tabs `ALL`, `영덕군보건소`, `세무사`, and `ATT` over the already-scoped two-folder payload; switching tabs does not trigger additional IMAP fetches or include `INBOX`; versioned `mail.js?v=4` and `app.js?v=252`; 23 frontend tests pass; served files are byte-synced | H3 static portal promoted; H4 synced and KaosBrain doctor passes |
...
| 2026-09-01 | 8 | Completed observation for the read-only personal PWA System status panel | Commit `997c5e8`; the Settings route fetches protected `GET /api/system/status` through Governor, displays KaosDiscoord/service/runtime status, offers a navigation-only `#brain` link, and hides the shell `+` on Settings so system writes are not exposed in PWA; versioned `styles.css?v=271` and `app.js?v=256`; 27 frontend tests and 369 containerized Governor/Discoord/Brain tests pass; H3 smoke verified healthy containers, served asset versions, unauthenticated 401, read-only internal status, and configured brain link; user confirmed the live PWA view on 2026-09-01 | Deployed to H3 and observation complete; H4 synced and KaosBrain doctor passes; production impact is read-only visibility only |
| 2026-09-02 | 6 | Added Documents Inbox Paperless reconciliation | Commit `806ef15`; Documents Inbox refresh can now ask Paperless for submitted task status, update the local intake record to `ARCHIVED` with a Paperless source link when OCR/import succeeds, mark `FAILED` when Paperless reports failure/revoked, and leave still-running tasks as `OCR PENDING`; versioned `documents.js?v=4` and `app.js?v=257`; 27 frontend tests, compose validation, deploy-helper syntax check, `git diff --check`, and 369 containerized Governor/Discoord/Brain tests pass; H3 smoke verified healthy containers, served asset versions, unauthenticated 401, and a non-mutating live Inbox read with zero records | Deployed to H3; expected production impact is local intake-state reconciliation only, with no document metadata/tag/delete mutation |
| 2026-09-02 | 6 | Made Documents Inbox rows actionable | Commit `c534d4b`; Inbox rows now open a local detail panel showing status, submitted/updated time, filename, size, Paperless task id, and SHA prefix; the panel offers `↻ STATUS` and, once archived, `OPEN DETAIL` into the PWA Paperless detail view plus `SRC`; uploads return to Inbox with the new/duplicate row selected; versioned `app.js?v=258`; 29 frontend tests, `git diff --check`, H3 served asset smoke, H4 sync, and KaosBrain doctor pass | Deployed to H3; static PWA only, no backend or document mutation change |
| 2026-09-02 | 6 | Added an explicit Documents metadata/tag confirmation gate | Commit `55e7a74`; archived Inbox records can now prepare a Paperless title/tag proposal from the PWA detail panel; previewing performs no Paperless write, shows `CONFIRM BEFORE APPLYING`, and final apply requires both a browser confirmation and a server-side `confirmed=true` flag before Paperless metadata is updated; versioned `app.js?v=259`; 30 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and 369 containerized Governor/Discoord/Brain tests pass; H3 smoke verified healthy containers, served asset version, metadata UI code, and unauthenticated 401 on the metadata proposal route; H4 synced to `244b621` and KaosBrain doctor passes | Deployed to H3; the PWA exposes only guarded Paperless title/tag writes for archived Inbox records, with no delete/retry/bulk metadata mutation |
| 2026-09-02 | 6 | Move Documents Inbox records to Archive after final metadata apply | Commit `4529df8`; final PWA metadata apply now includes the intake record id; Governor verifies that the record matches the Paperless document before writing metadata, marks the intake record `applied` after success, hides applied records from the Inbox board, and returns the PWA to Archive with the updated Paperless document selected; versioned `app.js?v=260`; 30 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and 369 containerized Governor/Discoord/Brain tests pass; H3 smoke verified healthy containers, served asset version, archive-switch code, record id handoff, and unauthenticated 401 on metadata apply | Deployed to H3; applied records remain in `/data/documents/intake.json` as audit history but no longer appear as Inbox work items |
| 2026-09-02 | 6 | Added guarded metadata editing to Documents Archive details | Commit `1617326`; Documents Archive detail now reuses the same title/tag metadata review form as Inbox: edit, `PREVIEW`, `CONFIRM BEFORE APPLYING`, then final browser-confirmed `APPLY`; archive edits call the same protected Paperless metadata endpoint without an intake record id, so they update Paperless but do not alter Inbox audit state; versioned `app.js?v=261`; JavaScript syntax check, 30 frontend tests, `git diff --check`, H3 served asset smoke, H4 sync, and KaosBrain doctor pass | Deployed to H3; PWA-only follow-up with no new backend route or Paperless write path beyond the existing confirmation-gated metadata endpoint |
| 2026-09-02 | 6 | Show readable Paperless tags in Documents detail and editor | Commit `4ad7e70`; Paperless document detail now resolves tag IDs to names when possible, keeps tag IDs if tag lookup is unavailable, includes readable `tags` in the Governor/PWA document contract, displays tags in Documents Archive detail metadata, and prefills the Archive metadata editor with `#tag` names; versioned `documents.js?v=5` and `app.js?v=262`; JavaScript syntax check, 30 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and 369 containerized Governor/Discoord/Brain tests pass; H3 smoke verified healthy containers, served asset versions, frontend tag display/editor code, and unauthenticated 401 on document detail | Deployed to H3; Archive rows remain uncluttered as number/date/title only, while tags live in detail/edit panels |
| 2026-09-02 | 6 | Added desktop open-list navigation for the main PWA | Commit `355b374`; main-profile desktop layout now keeps the mobile dropdown hidden on wide screens, renders the personal sections as an always-open Agenda/Calendar/Tasks/Supplies/Memos/Documents/Fax/Mail/Utils/Settings list in the squared left menu card, and pins the circular page-aware `+` in the card header area; versioned `styles.css?v=272` and `app.js?v=263`; JavaScript syntax check, 31 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; mobile/PWA dropdown behavior remains unchanged |
| 2026-09-02 | 6 | Aligned Documents upload and metadata form rows | Commit `58e5a20`; Documents upload `FILE`/`TITLE` rows and detail metadata editor `TITLE`/`TAGS` rows now share a fixed label column so their controls align cleanly in the archive-terminal UI; versioned `styles.css?v=273`; JavaScript syntax check, 32 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; CSS-only visual polish with no upload, Paperless metadata, or Governor behavior change |
| 2026-09-02 | 6 | Converted archive commands to bracketed text actions | Commit `91c5085`; archive-terminal command actions now render as lightweight colored text commands with CSS-generated brackets (`[ BACK ]`, `[ APPLY ]`, `[ SRC ]`, etc.), hover brightening/boldness, and no button fill/border; Fax/Mail mode selectors and Documents Archive/Inbox tabs remain boxed for selected-state clarity; versioned `styles.css?v=274`; JavaScript syntax check, 33 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; CSS-only visual polish with no action semantics or Governor behavior change |
| 2026-09-02 | 6 | Fixed desktop archive column alignment | Commit `5fea9b0`; desktop archive rows now use wider fixed lanes for `NO.` and `DATE`, keep Memos/long generated IDs clipped with ellipsis instead of bleeding into the date, and preserve one-line titles; versioned `styles.css?v=275`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; CSS-only desktop visual polish with no data/API behavior change |
| 2026-09-02 | 6 | Repositioned the desktop shell plus button | Commit `f391859`; desktop main PWA left-card `+` now anchors in the KaosGDD header band instead of floating beside the active menu row, with the menu list starting lower for breathing room; versioned `styles.css?v=276`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; CSS-only desktop visual polish with no action/API behavior change |
| 2026-09-02 | 6 | Reduced the shell plus circle size | Commit `be153cd`; the page-aware shell `+` keeps its glyph style and behavior but uses a smaller 36 px circular visual on mobile and desktop, with adjusted desktop header spacing; versioned `styles.css?v=277`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; CSS-only visual polish with no action/API behavior change |
| 2026-09-02 | 6 | Corrected personal portal default branding | Commit `85315f3`; `kaosgdd.net` startup/default HTML now identifies as `KaosGDD` instead of `Kaos Family`, including the page title, iOS web-app title, and initial visible fallback header; the `family.kaosgdd.net` host override still switches to `Kaos Family`; JavaScript syntax check, 35 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-HTML smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; HTML/test-only branding correction with no route/API behavior change |
| 2026-09-02 | 6 | Anchored the desktop shell plus to the left card | Commit `6509077`; desktop main menu picker no longer acts as the positioning parent for the shell `+`, so the button anchors to the left card/header instead of drifting beside the active menu row; versioned `styles.css?v=278`; JavaScript syntax check, 35 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 served-asset smoke, H4 sync, and KaosBrain doctor pass | H3 static portal promoted; CSS-only desktop visual fix with no action/API behavior change |
| 2026-09-02 | 6 | Cleaned the main Settings page layout | Commit `6272b37`; Main `kaosgdd.net` Settings now renders as a compact Archive Terminal-style settings board with KaosGDD overview, System Status, a compact Governor summary, and grouped automation/template sections; Family Settings remains on the family panel layout; versioned `styles.css?v=279` and `app.js?v=264`; JavaScript syntax check, 36 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, and H3 served-asset smoke pass | H3 static portal promoted; layout/organization cleanup only, with no route/API/write-policy behavior change; H4 synced to tracker commit `b93eef8` and KaosBrain doctor passes |
| 2026-09-02 | 6 | Restored Supplies through a governed PWA API | Commit `c226a56`; Personal Supplies no longer calls the retired legacy `/api/supplies` route; `family-portal` now routes `/api/supplies` to `governor-api`, Governor maps list/create/done/active/delete to the existing Radicale supplies `VTODO` profile, and the PWA Supplies page renders as an Archive Terminal board with `#/add-supply` opened by the shell `+`; versioned `navigation.js?v=4` and `app.js?v=265`; JavaScript syntax check, 39 frontend tests, Governor API syntax check, deploy-helper syntax check, compose validation, `git diff --check`, 369 containerized H3 tests, served-asset smoke, Governor health, unauthenticated `/api/supplies` 401, and internal read-only supplies payload smoke pass | H3 promoted; expected production impact is restoring personal Supplies list/add/done/delete via Governor while keeping retired `/embed/agenda-supplies` closed; H4 synced to tracker commit `f89f4ee` and KaosBrain doctor passes |
| 2026-09-02 | 6 | Hardened Supplies against stale client 404 state | Commit `ae3dbfc`; after the desktop PWA showed the new Supplies board while retaining an earlier `HTTP 404` error in client state, the Supplies loader now performs one automatic recovery retry for a cached error and still leaves manual `[ RETRY ]` for real backend failures; versioned `app.js?v=266`; JavaScript syntax check, 39 frontend tests, `git diff --check`, H3 static sync, served-asset smoke, and unauthenticated `/api/supplies` 401 smoke pass | H3 static portal promoted; frontend-only reliability fix with no API, Governor, or Radicale behavior change; H4 synced to tracker commit `8fed157` and KaosBrain doctor passes |
| 2026-09-02 | 6 | Removed stale edge 404 block for governed Supplies API | The live Caddy edge config still intercepted `kaosgdd.net /api/supplies*` with the old retired Supplies `respond 404` rule, so authenticated public PWA requests never reached the restored `family-portal` → Governor route; Caddy now keeps only retired `/api/capture` blocked, `/api/supplies*` falls through to the portal/Governor path, and `kaos-h3 edge-preflight` now rejects future `/api/supplies` interception; Caddy validate/reload, edge preflight, deploy-helper syntax check, Caddy-port `/api/supplies` 401 smoke, and `/api/capture` 404 smoke pass | Live edge fixed on H3; commit `2888aff` added the preflight guard and progress record; H4 synced to `2888aff` and KaosBrain doctor passes; no Governor, Radicale, or frontend behavior change |
| 2026-09-02 | 6 | Aligned desktop archive column headers with record rows | Desktop `NO. / DATE / TITLE` archive headers now compute their `ch` grid columns at the same font size as the row grid while keeping the header label text small, so Memos/Documents/Supplies/Fax/Mail board labels line up with the actual row lanes; versioned `styles.css?v=280`; JavaScript syntax check, 39 frontend tests, `git diff --check`, H3 static sync, and served CSS smoke pass | H3 static portal promoted; CSS-only visual alignment fix with no route/API/data behavior change; H4 synced to `b1c8e2c` and KaosBrain doctor passes |
| 2026-09-02 | 6 | Cleaned main Settings into a read-only control room | Commits `1590581` and `bd8ba6f`; Main `kaosgdd.net/#/settings` now renders as a compact read-only control room: overview, System Status, Governor summary, domain/source-of-truth map, and admin console links; removed the main Settings editor stack for Mail Organizer, generated calendar toggles, event presets, recurring tasks, and holiday sync while leaving family Settings and backend APIs untouched; Settings route now loads only system and Governor status on the main profile; added the missing `family-portal` nginx `location = /api/settings/status` proxy to `governor-api`; versioned `styles.css?v=281` and `app.js?v=267`; JavaScript syntax check, 40 frontend tests, deploy-helper syntax check, compose validation, `git diff --check`, H3 static sync, served asset smoke, and portal `/api/settings/status` JSON smoke pass | H3 static portal promoted; expected production impact is less clutter and fewer PWA write surfaces on main Settings, with no Governor/data migration; H4 synced to `bd8ba6f` and KaosBrain doctor passes |
| 2026-09-02 | 6 | Aligned desktop content top with left menu card | Main-profile desktop `.view` now has a specificity-matched media override so the earlier mobile/default `padding-top: 14px` no longer pushes every page content panel below the left navigation card; versioned `styles.css?v=282`; JavaScript syntax check, 40 frontend tests, and `git diff --check` pass | H3 static portal promoted; CSS-only shell alignment fix for all main desktop pages with no route/API/data behavior change; H4 synced to `40b7d14` and KaosBrain doctor passes |
| 2026-09-02 | 6 | Standardized archive board headers and status blocks | Fax, Mail, Supplies, Documents Archive, Documents Inbox, and Memos now share the visible `NO. / DATE / TITLE` archive column header; Supplies gained the missing header, Fax dropped the outdated `ID / DATE / REMOTE / TITLE` header, archive empty/loading/error messages now render as consistent rectangular board blocks, and Memos desktop view receives the same top-alignment override as other main pages; versioned `styles.css?v=283` and `app.js?v=268`; JavaScript syntax check, 41 frontend tests, `git diff --check`, H3 static sync, served asset smoke, and served header/CSS smoke pass | H3 static portal promoted; CSS/markup-only archive consistency pass with no route/API/data behavior change; H4 synced to `8bf20ce` and KaosBrain doctor passes |
...
| 2026-09-03 | 6 | Started Family Calendar contextual smart input | Commit `597f18f`; Family `#/add-event` now defaults to a selected-date smart textbox on the Family profile, keeps normal/manual and preset tabs available, previews slash/newline-separated day notes such as `연차/10:30 3교시 참관수업 / 2:30 스파예가`, and keeps the save action disabled until Brain parsing and Governor-confirmed Radicale writes are added; versioned `styles.css?v=289`, `translations.js?v=175`, and `app.js?v=276`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and Caddy-port served HTML smoke pass | H3 static portal promoted. UI-only first slice: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA default behavior change. H4 sync/doctor was attempted but blocked by DNS resolution failure for `h4`, `kaos-h4`, and `h4.local` |
| 2026-09-03 | 6 | Fixed Family smart input time ranges | Commit `1becf12`; Family smart event preview now parses explicit ranges like `12:30-14:00 메롱` as title `메롱`, start `12:30`, and end `14:00` instead of treating `-14:00` as part of the title; range separators `-`, `~`, `–`, and `—` are accepted, with an overnight guard for end times earlier than the start; versioned `app.js?v=277`; direct parser smoke for `연차 / 12:30-14:00 메롱`, JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served JS smoke, and portal-served JS smoke pass | H3 static portal promoted. UI/parser-only follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Added Family smart preview time nudges | Commit `7b5bba8`; timed Family smart-preview rows now include compact `-1h` and `+1h` controls that shift both start and end times in preview state, while all-day rows stay untouched and text edits reset stale per-row offsets; versioned `styles.css?v=290`, `translations.js?v=176`, and `app.js?v=278`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. Preview-only UI follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Changed Family smart preview nudges to end-time only | Commit `9b2d9f6`; the timed preview `-1h`/`+1h` controls now sit on the right side of each preview row and adjust only the inferred end time/duration, preserving the user-specified start time; shortening is ignored if it would make the end time less than or equal to the start; versioned `styles.css?v=291`, `translations.js?v=177`, and `app.js?v=279`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. Preview-only UI follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Labeled Family smart duration controls with current duration | Commit `dbb18b9`; the timed smart-preview controls now render as `<< 1시간 >>` with the center label derived from the current preview duration, so nudging the end time changes the label to values like `2시간` or `1시간 30분` while preserving the typed start time; versioned `styles.css?v=292`, `translations.js?v=178`, and `app.js?v=280`; JavaScript syntax checks, direct duration-label smoke, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. Preview-only UI follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Showed full range in Family smart preview time column | Commit `8d0802b`; timed smart-preview rows now show the full `start–end` range in the left time column, matching the row detail line after duration nudges, and the preview time column was widened to fit values like `12:30–14:30`; versioned `styles.css?v=293` and `app.js?v=281`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. Preview-only UI follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Themed the Family smart input textbox | Commit `237dd7c`; the Family smart-event textarea now uses the Family lavender/pink card treatment, softer focus ring, themed placeholder, and a scoped selector so other PWA forms remain unchanged; versioned `styles.css?v=294`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served CSS smoke, and portal-served CSS smoke pass | H3 static portal promoted. CSS-only UI follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Simplified the Family smart input field | Commit `e4e0831`; removed the visible `그냥쓰기` textbox label and its unused translation key, changed the smart textarea to sit directly in the card with an aria label, and made its width explicitly fill the card; versioned `styles.css?v=295`, `translations.js?v=179`, and `app.js?v=282`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. UI-only Family follow-up: no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Applied the Family global font to smart input | Commit `3b62b84`; the Family smart-event textarea now explicitly inherits the active Family profile font instead of falling back to the browser's default form-control font; versioned `styles.css?v=296`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served CSS smoke, and portal-served CSS smoke pass | H3 static portal promoted. CSS-only Family UI fix with no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 6 | Locked Family smart input resizing to vertical only | Commit `4d10eb8`; the Family smart-event textarea now keeps the full card width and disallows horizontal resizing by explicitly setting `resize: vertical`; versioned `styles.css?v=297`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served CSS smoke, and portal-served CSS smoke pass | H3 static portal promoted. CSS-only Family UI fix with no Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA behavior change |
| 2026-09-03 | 7 | Enabled confirmed Family smart calendar saves | Commit `4a01d3c`; Family smart input now changes from preview-only to `확인 후 저장`: the PWA builds deterministic event payloads from the reviewed preview rows, asks for browser confirmation with the parsed event list, posts each event through the existing `/api/calendar/events` Calendar Adapter route, clears the smart input after success, returns to Calendar, and warns explicitly if a multi-event save partially succeeds; versioned `translations.js?v=180` and `app.js?v=283`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. Reuses the existing Family host/Radicale adapter write boundary; no Brain API call, Governor operation, calendar schema change, or personal/main PWA behavior change; no synthetic live event was created during validation |
| 2026-09-03 | 7 | Fixed Family smart save button live enablement | Commit `5674b4b`; typing into the Family smart-event textbox now refreshes both the preview and the `확인 후 저장` button state, so the button enables as soon as at least one preview proposal exists; versioned `app.js?v=284`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served asset smoke, and portal-served asset smoke pass | H3 static portal promoted. Frontend state fix only; existing confirmed Calendar Adapter/Radicale write boundary unchanged |
| 2026-09-03 | 6 | Snapped Family smart end-time nudges to hour boundaries | Commit `df6d6e7`; Family smart-preview end-time nudges now keep one-hour default durations for half-hour starts such as `12:30–13:30`, then `>>` first snaps the end to the next clean hour (`14:00`) before advancing by whole hours (`15:00`, etc.); `<<` clamps positive offsets back to the original inferred end instead of overshooting earlier than intended; versioned `app.js?v=285`; JavaScript syntax checks, `git diff --check`, and 62 Family portal tests pass; H3 static sync, nginx validation, container-served JS smoke, and portal-served JS smoke pass | H3 static portal promoted. Frontend preview/save payload behavior only; no Brain API call, Governor operation, Radicale schema change, or personal/main PWA behavior change |
| 2026-09-03 | 7 | Added Family smart event AI preview hook | Commit `71bb039`; Family smart input now has an `AI 정리` preview action that posts selected-date text to `POST /api/calendar/smart-events/preview`; Calendar Adapter validates AI proposal shape, falls back to the server grammar parser when no AI URL is configured or when AI fails, and never writes during preview; the existing `확인 후 저장` flow remains the only Radicale write path through `/api/calendar/events`; versioned `translations.js?v=181` and `app.js?v=286`; JavaScript syntax checks, Python syntax check, `git diff --check`, 16 focused portal tests, and 21 focused Calendar Adapter tests pass; H3 served asset smoke and preview API smoke pass | H3 promoted. Initially ran grammar fallback until H4 Brain was wired; subsequent H3-to-H4 smoke now returns `source: ai` while retaining grammar fallback on Brain failure |
| 2026-09-03 | 7 | Added the KaosBrain smart calendar preview endpoint | Commit `c191b2a`; KaosBrain now exposes authenticated `POST /internal/calendar/smart-events/preview`, reuses the existing KaosBrain-OpenAI/OpenClaw planner, prompts for JSON preview-only Family calendar events, validates AI event shapes, and returns candidate events without any Governor/Radicale write capability; Calendar Adapter now supports direct `CALENDAR_SMART_EVENTS_AI_TOKEN` as well as token files for forwarding; deploy examples document H3-to-H4 wiring; Python syntax checks, `git diff --check`, focused Brain/Calendar Adapter tests, and the full Brain container test passed (`335 tests OK`) | H4 was reached via its Tailscale DNS name `kaosbrain` at `100.113.169.46`; KaosBrain was pulled to `f8e7de8`, configured with `KAOSBRAIN_CALENDAR_PREVIEW_API_TOKEN_FILE`, rebuilt/restarted, and direct H4 smoke plus H3 Family preview smoke both returned HTTP 200 with `source: ai` |
| 2026-09-03 | 7 | Accepted natural Family calendar note separators and paused active tuning | Commit `5d456e6`; Family smart calendar grammar fallback and the KaosBrain-OpenAI preview prompt now treat bundled day notes separated by slash, newline, comma, `+`, `&`, `그리고`, `그다음`, `그 다음`, `다음`, `또`, and `및` as separate event candidates; `app.js?v=287`; live H3-to-H4 smoke for `연차, 12:30 메롱 그리고 2:30 스파예가 + 18:00 저녁` returned `source: ai` with four preview events and one-hour timed defaults; 62 Family portal tests, focused Calendar Adapter parser tests, and the full Brain container test passed | H3 portal and Calendar Adapter promoted; H4 KaosBrain pulled/rebuilt/doctor healthy. Active Family Calendar smart-input tuning is paused unless real use reveals mistakes; next planned follow-ups are editable preview rows, optional default-duration settings, and Family Tasks smart textbox |
| 2026-09-03 | 7 | Added AI-assisted Paperless tag suggestions for Documents review | Commit `851c96d`; Personal Documents metadata review now has an `AI TAGS` action that asks Governor for Paperless document context, forwards title/OCR/current-tags plus the existing Paperless tag list to KaosBrain, and fills the editable tag field with matching existing tags only; manual tags are preserved, metadata `PREVIEW` and confirmed `APPLY` remain the only Paperless write path, and `app.js?v=288`; focused portal checks, H3 container tests (`369 tests OK`), and Brain image tests (`342 tests OK`) pass | H3 Governor and H4 KaosBrain deployed; live H3→H4 smoke returned `source: ai` with existing tag names only. Future tuning: improve the Paperless tag vocabulary and add archive-detail metadata editing if real review shows missing categories |
| 2026-09-03 | 4 | Named prompt-driven PWA workflows as AI Tasks | Documented **AI Tasks** as durable prompt/card workflows in the Kaos Feed target design and top-level architecture; every run should archive prompt, sources, preview, confirmation state, result, provider metadata, and errors while keeping Memos/Paperless/Radicale/mail/fax as the authoritative stores | Planning/docs-only update. First planned general AI Task remains `Official Doc -> Memo`, starting with pasted official URL/text if no reliable official-domain search provider is wired yet |
| 2026-09-03 | 7 | Implemented first AI Tasks archive workflow | Commits `8871b7a` + `ae9fa38`; Personal PWA gains an `AI Tasks` page and top-menu item. The first task type is `Official Doc -> Memo`: user enters a prompt plus official URL or pasted official text, Governor fetches/extracts source text when a URL is provided, H4 KaosBrain-OpenAI returns a memo draft only, Governor archives the preview record, and the PWA writes to Memos only after explicit `SAVE MEMO`, then marks the AI Task applied. Added H3 state storage at `/data/ai-tasks/archive.json`, an authenticated H4 preview endpoint, static asset bumps, nginx proxy routing for `/api/ai-tasks`, and tests for UI contract, Brain memo preview parsing, archive completion, and private-network URL blocking; focused checks and full H3 helper test (`369 tests OK`) pass | H3 Governor/static portal promoted; H4 KaosBrain pulled/rebuilt and doctor healthy at `ae9fa38`; H3 served asset smoke, H3 proxy smoke, H4 authenticated endpoint smoke, and H3→H4 temporary-archive smoke pass. This does not auto-file into Documents/Paperless and does not yet perform official-site search from a query alone; source URL/text is required for the first slice |
| 2026-09-03 | 6 | Kept comma-separated appointment details inside one Family smart event | Commit `c2befd4`; Family smart calendar parsing now treats comma as a weak separator: it splits only when the next segment clearly starts a new event with a time, while detail lists such as `초음파,피검사,엑스레이` remain in the same appointment title. KaosBrain-OpenAI prompt now carries the same rule and the exact `반차/ 1:40 영대병원-초음파,피검사,엑스레이` example; versioned `app.js?v=290`; focused parser/UI checks and full H3 helper test (`369 tests OK`) pass | H3 static portal promoted; Calendar Adapter restarted; H4 KaosBrain pulled/rebuilt and doctor healthy. Live H3 preview smoke for the example returns exactly two rows in both grammar and AI modes: all-day `반차` and timed `영대병원-초음파,피검사,엑스레이` |
| 2026-09-03 | 6 | Fixed AI Tasks preview archive permission and source-page errors | Commit `6b091ea`; diagnosed `ai_task_preview_failed` as a H3 archive write `PermissionError`: H4 returned the AI draft, but Governor could not write `/data/ai-tasks/archive.json` because the bind-mounted `ai-tasks` directory was root-owned. The H3 helper now corrects state directory group ownership/mode during `family-up`, Governor converts archive write failures to `ai_task_archive_write_failed`, and the PWA shows actionable AI Task errors. Also detects KDCA-style Korean missing/error pages such as `알림메세지 / 메뉴이(가) 존재 하지 않습니다.` as `ai_task_source_not_found`; versioned `app.js?v=291`; focused UI/parser checks and full H3 helper test (`369 tests OK`) pass | H3 Governor/static portal promoted; archive write smoke passes; served assets show `app.js?v=291`; live KDCA section URL check now returns `ai_task_source_not_found`. The KDCA URL used in the screenshot is not a valid article/source page, so the user should use a specific article URL or paste source text |
| 2026-09-03 | 6 | Added PDF upload as an AI Tasks source option | Commit `5f2c0dc`; `Official Doc -> Memo` now accepts URL, pasted text, or a selected PDF. The PWA posts multipart form data only when a PDF is selected; Governor validates PDF filename/signature/size, extracts embedded text with `pypdf`, sends only extracted text to H4 KaosBrain-OpenAI, archives the preview source as `type: pdf` without storing source text, and still writes to Memos only after explicit `SAVE MEMO`; versioned `app.js?v=292`; focused UI checks and full H3 helper test (`369 tests OK`) pass | H3 Governor/static portal promoted; served assets show `app.js?v=292`; live PDF extraction smoke passes; live PDF→H4 preview smoke with a temporary archive returns `source.type=pdf`, `status=previewed`, and an AI memo draft. This handles text-based PDFs; scanned/image-only PDFs still need OCR/Paperless first or pasted source text |
| 2026-09-03 | 7 | Expanded AI Tasks into a general web-search workbench | Commit `0c93598`; Personal AI Tasks now defaults to a tabbed `WEB SEARCH` task beside the existing `DOC -> MEMO` source/PDF flow. H3 Governor exposes `POST /api/ai-tasks/web/preview`, derives the H4 route from the existing AI Tasks URL when `AI_TASKS_WEB_BRAIN_URL` is blank, validates/sanitizes H4 results, and archives read-only result cards with source lists; H4 KaosBrain exposes authenticated `POST /internal/ai-tasks/web/preview` backed by OpenAI Responses API `web_search`; the PWA shows result/source cards with optional `[ copy ]` and confirmed `[ save memo ]` actions only; `app.js?v=293`; focused JS/syntax checks, H3 Docker suite (`369 tests OK`), and Brain Docker suite (`351 tests OK`) pass | H3 Governor/static portal promoted and H4 KaosBrain deployed/doctor healthy at `0c93598`; served static smoke confirms `app.js?v=293`. Live H4 no-cost route smoke currently returns `kaosbrain_web_search_not_configured`, so real web searching requires installing `KAOSBRAIN_OPENAI_API_KEY_FILE` or `OPENAI_API_KEY_FILE`; without that secret the PWA shows a clear configuration error and performs no external write |
| 2026-09-03 | 7 | Routed AI Tasks web search through official health allowlist | Commit `0b2167c`; Personal AI Tasks `WEB SEARCH` now uses KaosBrain-OpenAI/OpenClaw to create a structured search job, then Governor searches/fetches only allowlisted Korean official/public health domains before sending source excerpts back to KaosBrain-OpenAI for summary/reasoning. Added H4 internal routes `/internal/ai-tasks/official-web/plan` and `/internal/ai-tasks/official-web/summarize`, H3 official-search adapters for MOHW/KDCA/MFDS/HIRA public search pages, broader source-host allowlist, PDF/text source fetching under the same host policy, clearer PWA error labels, and `app.js?v=294`; local syntax checks, focused Brain web-task tests, H3 Docker suite (`369 tests OK`), and Brain Docker suite (`353 tests OK`) pass | H3 Governor/static portal promoted and H4 KaosBrain deployed/doctor healthy at `d60dd42`; served static smoke confirms `app.js?v=294`, H3 API proxy smoke returns the expected Cloudflare gate, and H4 no-generation route smokes confirm `/official-web/plan` and `/official-web/summarize` validation. Not broad web browsing: Governor rejects arbitrary hosts and archives read-only results with checked date, plan, and sources. Search adapters initially cover `mohw.go.kr`, `kdca.go.kr`, `mfds.go.kr`, and `hira.or.kr`; other allowlisted national/affiliated health domains may appear as fetched source links but are not all searched directly yet |
| 2026-09-03 | 7 | Improved HIRA medicine benefit discovery for AI Tasks | Commits `89dae70` and `b66521d`; HIRA generic search was replaced with a dedicated 보험인정기준 POST-search adapter, because the old generic HIRA search URL returned dead/noisy results and medicine criteria may be indexed by ingredient or class instead of Korean brand name. Governor now expands examples such as `알모그란/알모트립탄 -> Almotriptan/편두통 치료제` and `글리아티린 -> Choline alfoscerate/콜린알포세레이트`, ranks newer HIRA criteria ahead of old product-specific rows, filters search/menu noise, returns only HIRA candidates when HIRA is the preferred source, and tells KaosBrain-OpenAI to add known ingredient/class alternate queries for Korean medicine brands. Validation found live HIRA candidates for `알모그란정 급여기준`, with 2024 `편두통 치료제` first and 2013 `Almotriptan 경구제 (품명: 알모그란정)` second; extracted HIRA source text includes `Almotriptan` and the 2024 criteria. Syntax checks, H3 Docker suite (`369 tests OK`), and Brain Docker suite (`353 tests OK`) pass | H3 Governor redeployed and H4 repo synced to `b66521d`; live H3 smoke returns only the two HIRA candidates and extracts the 2024 HIRA source text. Production impact is search/source selection only. AI Tasks remain read-only until the user explicitly saves/copies elsewhere, and Governor still rejects non-allowlisted hosts |
| 2026-09-04 | 7 | Added 약학정보원 medicine dictionary enrichment for AI Tasks search | Commit `a44e602`; Governor now treats `health.kr` as a read-only medicine dictionary helper for product-name, ingredient-name, MFDS classification, and category hints. For medicine benefit queries, Governor can first search health.kr product JSON with CSRF/session handling, fetch the selected drug-detail JSON, derive English/Korean ingredient and category terms, and then search HIRA 보험인정기준 with those enriched queries. Focused validation proves an unknown brand can be expanded through health.kr into an ingredient before HIRA search, and live smokes return HIRA criteria for both `알모그란정 급여기준` and `종근당글리아티린 요양급여기준` (`choline alfoscerate 경구·시럽·주사제` first). Syntax checks and H3 Docker suite (`369 tests OK`) pass | H3 Governor redeployed; live H3 smoke returns HIRA-only candidates for both test medicine-benefit queries. `health.kr` is not treated as the final authority for insurance-benefit decisions; if HIRA criteria are found for a HIRA-preferred query, only HIRA candidates are returned to the summarizer. Future tuning: cache health.kr lookup results if external latency becomes annoying |
| 2026-09-04 | 7 | Added chart-note guidance to medicine benefit AI Task answers | Commit `48439ee`; KaosBrain-OpenAI official-web summaries now explicitly include a source-bounded `차트 기재 추천` section for Korean medicine insurance/benefit questions such as 급여기준, 요양급여, 본인부담, 투여조건, or 삭감. The section should provide copyable Korean chart-note examples with placeholders and a checklist of source-supported clinical facts to verify, while refusing chart-note guidance if the fetched criteria are insufficient. Focused Brain prompt tests pass and Brain Docker suite passes (`354 tests OK`) | H4 Brain deployed and doctor passes at `48439ee`. This is answer-format guidance only: no EMR write, no PWA write expansion, and no change to Governor source fetching or HIRA/health.kr search |
| 2026-09-04 | 6 | Planned indexed PDF textbook source tier for AI Tasks | Documented server-side textbook PDFs as a later AI Task source tier: extract/OCR once, chunk by page/section, index locally, send only relevant excerpts to KaosBrain-OpenAI, cite filename/page/section/hash, and prefer current official/guideline sources over textbook background when they conflict | Planning/docs-only. Implementation order stays web sources first, then local indexed PDF references. Copyright guardrail: summaries and short citations only, not long copied passages |
| 2026-09-04 | 7 | Merged AI Tasks type tabs into one prompt/source composer | Commit `6b6a2a8`; Personal PWA AI Tasks now has one `AI TASK` composer instead of separate `WEB SEARCH` and `DOC -> MEMO` tabs. Prompt-only submissions use the official-source web-search path, while adding a PDF, URL, or pasted text uses the existing source/document memo-draft path. The backend routes remain separate behind the UI, and result cards still require explicit `SAVE MEMO` before writing to Memos | H3 static portal promoted; served HTML points to `app.js?v=295`, served JS contains the unified composer, nginx validation passes, frontend tests pass (`65/65`), and H3 helper tests pass (`369 OK`). This is a UI routing simplification only: no new write capability, no broader source allowlist, and no change to H4 Brain behavior |
| 2026-09-04 | 6 | Made AI Task archive rows open their stored result detail | Commit `8bddec7`; Personal PWA AI Task archive rows now behave like the other archive boards: tapping a row selects it, renders the archived result/memo draft from the existing list payload, provides `[ BACK ]` and `[ copy ]`, and shows `SAVED` instead of offering a duplicate save for already-applied tasks. Applied web-search tasks fall back to the preserved memo draft text when the completion receipt has replaced the result payload | H3 static portal promoted; served HTML points to `app.js?v=296`, served JS contains archive open/back/detail markers, nginx validation passes, frontend syntax/focused AI Tasks/full family-portal tests pass (`65/65`), and H3 helper tests pass (`369 OK`). UI/read-only archive behavior only; no backend route, source fetching, Brain, or write-policy change |
| 2026-09-04 | 6 | Extended the Family Portal AI Tasks proxy timeout | Commit `f1c1c51`; diagnosed screenshot `HTTP 504` as the portal nginx default proxy timeout expiring while Governor continued a long official-source search/fetch/summarize request, evidenced by late Governor `BrokenPipeError` after H4 Brain returned successful plan/summarize responses. The family-portal nginx route for `/api/ai-tasks` now keeps `proxy_read_timeout 180s` and `proxy_send_timeout 180s`, with deploy-helper and frontend tests guarding the setting | H3 family portal config promoted; live nginx validates, `services-preflight` passes, and the running container config shows both AI Tasks timeout directives. Frontend tests pass (`66/66`), focused AI Tasks proxy-timeout test passes, and H3 helper tests pass (`369 OK`). This is a timeout/path reliability fix only; it does not broaden AI source access or add new writes. Long-term option remains async job + polling if official searches routinely exceed outer edge limits |
| 2026-09-04 | 6 | Collapsed optional AI Task source fields under Details | Commit `a240712`; Personal PWA AI Task composer now keeps `PROMPT` and `PDF` visible while optional `URL` and source `TEXT` fields live inside a default-collapsed inline `DETAILS` disclosure, using the same compact bracket-command visual language as the rest of the archive UI. Cache-busted static assets to `styles.css?v=299` and `app.js?v=297` | H3 static portal promoted; live nginx validates, served HTML points to `styles.css?v=299` and `app.js?v=297`, served JS contains `aiTaskSourceDetails` / `DETAILS`, and served CSS contains the disclosure styling. Validation passes: `node --check apps/family-portal/app.js`, focused AI Tasks UI tests, full family-portal tests (`66/66`), `git diff --check`, and H3 helper tests (`369 OK`). UI-only trim; no backend route, source policy, Brain behavior, or write capability changed |
| 2026-09-04 | 7 | Made AI Tasks asynchronous with richer result details and stronger official search | Commit `8001ef5`; Personal PWA submits the unified AI Task composer to a new `POST /api/ai-tasks/run` endpoint that creates a `running` archive card immediately, then Governor finishes the same card in a background worker as `previewed` or `failed`. The PWA polls running cards, renders explicit running/failed panels, and shows completed web-search details with plan metadata and a host-labeled source list. Governor archive records now accept the `running` lifecycle, preserve source privacy by keeping PDF/text payloads in memory for the worker rather than storing raw source text, and retains the old preview endpoints for compatibility. Official search now auto-prioritizes HIRA for medicine-benefit prompts even if Brain omits the preference, and adds a direct MFDS 의약품통합정보시스템 (`nedrug.mfds.go.kr`) product-info search adapter | H3 Governor API and static portal promoted; served HTML points to `styles.css?v=300` and `app.js?v=298`, served JS contains `/api/ai-tasks/run` / `AI TASK RUNNING`, served CSS contains `aiTaskSources`, `aiTaskPlan`, and `archiveNotice`, and unauthenticated live route smoke returns the expected Cloudflare 401 rather than 404. Validation passes: `node --check apps/family-portal/app.js`, full family-portal tests (`66/66`), `git diff --check`, H3 helper tests (`369 OK`), and focused Governor AI Task Docker tests (`16/16`). This is still read/confirm-only: AI Tasks may copy or explicitly save to Memos, but no automatic Memos, Paperless, EMR, mail, or fax writes were added |
| 2026-09-04 | 6 | Tightened Documents metadata review after real use | Commit `d1b3a7c`; Documents detail now keeps existing Paperless tags prefilled in the metadata editor instead of resetting them on open, shows `File` and explicit `Tags: none` in detail metadata, keeps the updated document selected after a confirmed metadata apply, locally updates the archive row title/metadata, and filters already-applied intake records client-side as a second guard in addition to the Governor inbox filter | H3 static portal promoted; served HTML points to `documents.js?v=6` and `app.js?v=299`, served JS contains the applied-record filter and tag-prefill/detail display code. Validation passes: `node --check apps/family-portal/app.js`, `node --check apps/family-portal/documents.js`, full family-portal tests (`67/67`), `git diff --check`, and H3 helper tests (`369 OK`). No new backend route or write capability; Paperless metadata writes still require preview plus explicit browser confirmation |
| 2026-09-04 | 6 | Kept uploaded Paperless documents visible in Archive while Inbox tracks review | Commit `238d219`; Documents Archive now treats Paperless as the source-of-truth list and overlays the local intake review queue onto matching archive rows with a small `REVIEW`/`FAILED` marker instead of implying those documents are hidden outside Inbox. Opening a marked Archive detail carries the matching inbox record id into the existing metadata review form, so a confirmed metadata `APPLY` from Archive can mark the Inbox work item applied and remove it from the review queue | H3 static portal promoted; served HTML points to `styles.css?v=301` and `app.js?v=300`, served JS contains the archive review marker and selected-review record handoff. Validation passes: `node --check apps/family-portal/app.js`, focused Documents frontend tests (`13/13`), full family-portal tests (`69/69`), `git diff --check`, and H3 helper tests (`369 OK`). No new backend route; Paperless writes remain the existing preview plus explicit confirmation metadata path |
| 2026-09-04 | 6 | Added supplemental general-web follow-up from AI Task results | Implementation commit `7177829`; Personal AI Tasks still starts prompt-only work through the official/allowlisted health-source pipeline, but completed official-result cards now expose `[ SEARCH WEB ]` to run a separate supplemental general-web search through the existing H4 KaosBrain web endpoint. Governor archives those follow-ups as `kind=general_web` with `source.type=general_web_search`, and the PWA labels them `GENERAL WEB CONTEXT`, lists sources, and warns that important decisions should still be verified against official sources; static JS cache bumped to `app.js?v=301` | H3 Governor/static portal promoted; served HTML points to `app.js?v=301`, served JS contains `general-web`, `data-ai-task-general-web`, and `GENERAL WEB CONTEXT`, and proxy smoke reaches the Cloudflare auth gate instead of 404. Validation passes: `node --check apps/family-portal/app.js`, `python3 -m py_compile apps/governor/src/kaos_governor/api.py`, focused AI Tasks UI tests, full family-portal tests (`69/69`), `git diff --check`, and H3 helper tests (`369 OK`). This is read-only except the existing optional `SAVE MEMO` action; broad web output is archived separately and does not modify Paperless, mail, fax, calendar, tasks, or Documents |
| 2026-09-04 | 6 | Enabled H4 OpenClaw/OAuth as the broad AI Tasks web-search backend | Implementation commit `0e5f9b8`; H4 OpenClaw config now enables `tools.web.search` with provider `codex`, `openaiCodex.mode=live`, and `tools.web.fetch` with readability. KaosBrain `/internal/ai-tasks/web/preview` still prefers a direct `KAOSBRAIN_OPENAI_API_KEY_FILE` when present, but now falls back to KaosBrain-OpenAI via OpenClaw/OAuth when no API key is configured, requiring structured JSON with title/content/sources for PWA rendering; PWA error text no longer implies API-key-only setup; static JS cache bumped to `app.js?v=302` | H4 Brain deployed and H3 static portal promoted; served HTML points to `app.js?v=302`, H4 doctor passes, and live H4 internal route smoke returns HTTP 200 with `source=openclaw-web-search` and a source list. Validation passes: `node --check apps/family-portal/app.js`, focused AI Tasks UI tests, full family-portal tests (`69/69`), local Brain web/OpenClaw tests (`31 OK`, `7 skipped`), Brain helper Docker suite (`356 OK`), and `git diff --check`. Production impact is read-only web-search capability through OpenClaw OAuth; no automatic Memos/Paperless/Radicale/mail/fax writes |
| 2026-09-04 | 6 | Quieted generic unread mail attention and fixed watched folder matching | Implementation commit `f3b8920`; Personal Mail still keeps the `UNREAD` cleanup tab and batch read/delete workflow, but shell attention now loads and evaluates the watched target mail board (`영덕군보건소`, `세무사`) instead of generic unread mail. Frontend mail filtering now treats nested IMAP mailbox paths such as `각종공문/영덕군보건소` as a match for the `영덕군보건소` tab, preventing fetched target messages from being filtered out client-side; static cache bumped to `mail.js?v=7` and `app.js?v=303` | H3 static portal promoted; served HTML points to `mail.js?v=7` and `app.js?v=303`; the same-origin Mail API route reaches Governor/Cloudflare auth instead of static 404; a direct Governor read-only target-folder probe returns `ok=True`, `mailboxCount=2`, and sampled target messages in both watched mailboxes. Validation passes: `node --check apps/family-portal/app.js`, `node --check apps/family-portal/mail.js`, focused Mail/attention/navigation tests, full family-portal tests (`69/69`), `git diff --check`, and H4 fast-forward/doctor after the implementation push. This is UI/read-only attention behavior only: no mail is marked read/deleted unless the user explicitly uses the existing `UNREAD` tab APPLY flow |
| 2026-09-04 | 7 | Added treatment-option source routing for AI Tasks web search | Implementation commit `8c3da6a`; prompt-only AI Tasks can now be classified as `treatment_options`, causing KaosBrain to request source-bounded treatment/guideline summaries and Governor to prioritize trusted treatment sources such as 국가건강정보포털, 국가정신건강정보포털, PubMed, NIH/NINDS, NICE CKS, and AASM while keeping `health.kr` enrichment limited to medicine/drug-benefit lookups. Live probe for `하지불안증후군 치료 옵션` ranks current AASM/PubMed guideline results above older/noisy PubMed matches. The architecture doc also records the later indexed PDF textbook source tier: server-side PDFs should be extracted/OCRed once, chunked with page/section/hash metadata, locally indexed, and used only as cited background after current official/guideline sources | H3 Governor API/worker restarted and H4 KaosBrain pulled/rebuilt/doctor healthy. Validation passes: Python syntax checks, frontend tests (`78/78`), `git diff --check`, H3 helper tests (`369 OK`), and Brain helper tests (`358 OK`). Production impact is read-only AI Task search/result quality only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes |
| 2026-09-04 | 7 | Fixed AI Tasks treatment answers that only saw PubMed titles | Implementation commit `1d0526d`; diagnosed the unhelpful BPPV screenshot as PubMed fetches returning a `Cookies must be enabled` page, leaving KaosBrain with titles/URLs instead of abstracts. Governor now fetches PubMed article metadata/abstracts through NCBI E-utilities from the PMID, skips PubMed records without usable abstracts instead of forwarding cookie/search chrome, upgrades treatment-option prompts from accidental `summary` plans to `treatment_options`, adds BPPV/이석증 aliases, diversifies source candidate collection across first/alternate queries, and ranks practical guideline/review records above adherence/barrier/navigation noise. Added AAO-HNS/ENTNET to the treatment-source allowlist/adapters | H3 Governor API/worker restarted and H4 KaosBrain pulled/rebuilt/doctor healthy. Validation passes: Python syntax checks, frontend tests (`78/78`), `git diff --check`, H3 helper tests (`369 OK`), and Brain helper tests (`358 OK`). Live BPPV probe now returns `task=treatment_options` and PubMed abstract text with the AAO-HNS BPPV clinical practice guideline record ranked first |

| 2026-09-05 | 6 | Preserved AI Task source findings when Brain summary fails | Implementation commit `fe9af19`; Governor now retries transient KaosBrain/OpenClaw summary failures once, preserves the structured official-search plan and fetched source links inside failed archive records when sources were found, and creates a partial result card that can be copied or used to launch supplemental general web search instead of leaving only `HTTP 502/504`-style errors. Treatment-option search also expands Korean/English disease prompts such as `하지불안증후군` into guideline/treatment query variants before scanning trusted health sources | Validation passes: JS syntax check, Python compile checks, `git diff --check`, and focused Governor AI Task Docker tests (`25 OK`). Production impact remains read-only search/archive behavior only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes |

| 2026-09-05 | 7 | Implemented Harrison textbook background for AI Tasks | Implementation commit `f172322`; Governor can now search a read-only local SQLite FTS index for Harrison's Principles of Internal Medicine, 22e, using Korean/English clinical aliases such as `하지불안증후군 -> restless legs syndrome`, `이석증/BPPV`, and common chronic diseases. Prompt-only AI Tasks still prioritize current official/guideline web sources, but can attach up to three local textbook excerpts as a separate `textbookSources` tier when available; KaosBrain receives those separately as background clinical context and the PWA renders them under `TEXTBOOK BACKGROUND`. H3 compose mounts `${KAOS_ROOT}/reference/textbooks` read-only at `/data/textbooks` and documents the override env knobs | Validation passes: real local index smoke for `하지불안증후군`, BPPV, and 당뇨; JS syntax and full frontend tests (`80/80`); focused Brain web-task tests (`9 OK`, `3 skipped` locally); focused Governor AI Task Docker tests (`27 OK`); H3 helper tests (`369 OK`); Brain helper tests (`359 OK`). Production impact remains read-only source retrieval and archived result display only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes. Extraction caveat: the initial Harrison PDF pass produced a usable page index but some low-text/error pages remain candidates for later Poppler/OCR recovery |

| 2026-09-05 | 6 | Added AAFP to AI Tasks treatment sources | Implementation commit `0b9d76c`; Governor now treats American Family Physician / `aafp.org` as an allowlisted treatment-options source and discovers condition collection pages from the AAFP sitemap, while KaosBrain-OpenAI planning hints include `www.aafp.org` for treatment/guideline prompts. The architecture doc now lists AAFP beside PubMed/NCBI, NIH/NINDS, NICE CKS, and professional society guideline pages | Validation passes: Python compile checks, focused Brain web-task tests (`9 OK`, `3 skipped` locally), focused Governor AI Task Docker tests (`28 OK`), H3 helper tests (`369 OK`), Brain helper tests (`359 OK`), `git diff --check`, and a live sitemap/candidate smoke where `asthma treatment options` ranked AAFP asthma collection pages before PubMed. Production impact remains read-only source retrieval/archive quality only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes. Current caveat: AAFP discovery uses sitemap topic/collection pages first; older AFP article archives may still require direct URLs or a later richer index |

| 2026-09-05 | 7 | Added Korean specialty society/public source routing for AI Tasks | Implementation commit `90558d3`; Governor now extends official-source treatment search with trigger-routed Korean specialty/public sources including NECA/guideline-center hosts, 국가암정보센터, 희귀질환 헬프라인, E-Gen, 국립재활원, and specialty society pages for hypertension/cardiology, diabetes, nephrology, hepatology, respiratory/infectious disease, ENT/vertigo, neurology, headache, stroke, pediatrics, pediatric infection, and pediatric allergy/asthma. KaosBrain-OpenAI planning hints now name representative Korean society domains, while Governor honors explicit `preferredDomains` instead of silently broadening them. Drug/product lookup intent is kept separate from disease treatment intent so `알모그란정 제품정보` stays on health.kr/NEDrug-style sources rather than drifting into AAFP/PubMed treatment search | Validation passes: Python compile checks, focused Brain web-task tests (`9 OK`, `3 skipped` locally), focused Governor AI Task Docker tests (`30 OK`), `git diff --check`, and live candidate smokes where `편두통 예방치료 옵션` ranks 대한두통학회 guideline records first, `소아 천식 치료 지침` ranks 대한소아알레르기호흡기학회 guideline page first, and `알모그란정 제품정보` ranks 약학정보원 first. Production impact remains read-only source retrieval/archive quality only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes |

| 2026-09-05 | 7 | Included Korean benefit criteria in treatment-option AI Tasks | Implementation commit `bd291b2`; Governor now searches HIRA 보험인정기준 as a secondary Korea-specific applicability layer for `treatment_options` prompts, placing HIRA benefit-query variants such as condition `치료제`, `급여기준`, `요양급여기준`, and `보험인정기준` ahead of generic treatment queries while keeping direct medicine-benefit prompts HIRA-primary. Brain summary guidance now asks for a distinct `한국 급여기준 확인` subsection when HIRA criteria are fetched. The specialty-source filter also rejects unrelated society menu/resource pages such as generic 교육자료, 위원회 자료실, and 전공의 수련지침서 unless they carry real treatment/context signal | Validation passes: Python compile checks, Brain web-task tests (`9 OK`, `3 skipped` locally), focused Governor AI Task Docker tests (`32 OK`), `git diff --check`, and live candidate smokes where `편두통 예방치료 옵션` returns 대한두통학회 guideline records plus HIRA `편두통 치료제`, `하지불안증후군 치료 옵션` no longer returns generic 대한신경과학회 menu pages and falls through to PubMed treatment/guideline records, and `알모그란정 급여기준` stays HIRA-primary. Production impact remains read-only source retrieval/archive answer quality only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes |

| 2026-09-05 | 6 | Fixed stale family-portal upstream DNS after Governor restarts | Implementation commit `20769f7`; diagnosed Mail/Fax 502s as `family-portal` nginx holding the old Docker IP for `governor-api` after the Governor container was recreated during deploy. The portal nginx config now uses Docker DNS (`127.0.0.11`) with dynamic `$governor_api` and `$calendar_adapter` upstream variables for governed API and calendar-adapter routes, and deploy preflight now requires those dynamic upstreams | Validation passes: deploy-helper syntax check, nginx config syntax check, `git diff --check`, and live H3 probes after restarting `family-portal`: Mail target messages, Mail unread, Mail attention, Fax all, and Fax failed all reach Governor instead of nginx 502. Production impact is routing reliability only; no Mail/Fax/Paperless/System data mutation or PWA write-surface change |

| 2026-09-05 | 7 | Added HIRA HWPX attachment extraction to AI Task official sources | Implementation commit `f48f17f`; Governor now detects HIRA 보험인정기준 detail attachments exposed through `Header.goDown` / `Header.goDown1`, downloads them through the allowlisted HIRA `/download.do` endpoint, and extracts text from `.hwpx` zip/XML attachments before sending the source set to KaosBrain-OpenAI. Direct `.hwpx` and `.hml` official-source URLs are supported, text PDFs continue to work, and legacy binary `.hwp` is explicitly left unsupported until a real server-side extractor is configured | Validation passes: Python compile checks, `git diff --check`, focused Governor AI Task Docker tests (`33 OK`), H3 helper suite (`369 OK`), live HIRA source smoke for `고시 제2024-178호 편두통 치료제.hwpx`, and live `알모그란정 급여기준` candidate/fetch smoke showing `Almotriptan` criteria in the fetched source text. Production impact remains read-only source retrieval/archive quality only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes |

| 2026-09-05 | 6 | Added Kaplan psychiatry textbook discovery for AI Tasks | Implementation commit `a3b5a72`; Governor textbook search now discovers every read-only SQLite FTS index under `/data/textbooks/*/index/*.index.sqlite` when the legacy Harrison path remains configured, while preserving `AI_TASK_TEXTBOOK_INDEX_PATH` as an explicit single-index override. Kaplan & Sadock's Synopsis of Psychiatry 12e is recognized from metadata/fallbacks and Korean psychiatry prompts such as `조현병`, `양극성`, `우울증`, `공황`, and related terms expand to English textbook aliases and prefer Kaplan before general medicine sources | Validation passes: `git diff --check`, H3 helper suite (`369 OK`), focused multi-index/Korean-alias tests, and live mounted smokes showing both Harrison and Kaplan indexes discovered with Kaplan excerpts returned for `조현병 치료`, `양극성장애 치료`, and `우울증 치료`. Production impact remains read-only textbook background retrieval only; no automatic Memos, Paperless, EMR, mail, fax, calendar, or task writes. Copyright guardrail remains summary/excerpt/citation use only, not wholesale textbook redistribution |
