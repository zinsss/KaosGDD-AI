...
| 2026-09-02 | 6 | Fixed stale Documents Inbox rows after metadata apply | Commit `127e88c`; after final metadata apply from Documents Inbox, the PWA now removes the applied intake row from local `inboxItems`, clears the selected Inbox item, forces the next Inbox view to re-read backend state, refreshes the shell attention marker, and then returns to Archive with the updated Paperless document selected; versioned `app.js?v=271`; JavaScript syntax check, 48 frontend tests, `git diff --check`, H3 static sync, and served JS smoke pass | H3 static portal promoted; frontend stale-state fix only. Backend confirmation-gated Paperless metadata apply and intake `status=applied` behavior were already covered and unchanged; H4 sync pending |
