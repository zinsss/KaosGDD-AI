...
| 2026-09-02 | 6 | Repositioned the desktop shell plus button | Desktop main PWA left-card `+` now anchors in the KaosGDD header band instead of floating beside the active menu row, with the menu list starting lower for breathing room; versioned `styles.css?v=276`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; CSS-only desktop visual polish with no action/API behavior change |
