...
| 2026-08-31 | 6 | Rebalanced the personal compact header with the flat page selector on the left, a larger `KaosGDD` wordmark on the right, and both vertically centered in the same 44 px row | Commits `6208dab` and `b315293`; 11 focused portal tests passed; deployed HTML/CSS hashes match repository sources; H4 synced and doctor healthy | Presentation-only follow-up; Family header, routes, APIs, data, and Brain runtime remain unchanged |
