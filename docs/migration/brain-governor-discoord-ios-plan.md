...
| 2026-09-01 | 6 | Implemented read-only PWA Mail detail body and attachment metadata | Added protected Governor `GET /api/mail/messages/{uid}?mailbox=...` using read-only IMAP `BODY.PEEK[]`, plus PWA detail loading/error states and attachment metadata display; versioned `mail.js?v=2` and `app.js?v=249`; tests updated | Local validation in progress; production promotion pending |
