...
| 2026-09-03 | 6 | Added PDF upload as an AI Tasks source option | Commit `TBD`; `Official Doc -> Memo` now accepts URL, pasted text, or a selected PDF. The PWA posts multipart form data only when a PDF is selected; Governor validates PDF filename/signature/size, extracts embedded text with `pypdf`, sends only extracted text to H4 KaosBrain-OpenAI, archives the preview source as `type: pdf` without storing source text, and still writes to Memos only after explicit `SAVE MEMO`; versioned `app.js?v=292`; focused UI checks and full H3 helper test (`369 tests OK`) pass | Promotion pending. This handles text-based PDFs; scanned/image-only PDFs still need OCR/Paperless first or pasted source text |
