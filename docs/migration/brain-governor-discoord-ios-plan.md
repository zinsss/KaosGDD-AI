...
| 2026-09-03 | 6 | Started Family Calendar contextual smart input | Family `#/add-event` now defaults to a selected-date smart textbox on the Family profile, keeps normal/manual and preset tabs available, previews slash/newline-separated day notes such as `연차/10:30 3교시 참관수업 / 2:30 스파예가`, and keeps the save action disabled until Brain parsing and Governor-confirmed Radicale writes are added; versioned `styles.css?v=289`, `translations.js?v=175`, and `app.js?v=276` | UI-only first slice. No Brain API call, Governor operation, Radicale write, calendar schema change, or personal/main PWA default behavior change |
