...
| 2026-09-04 | 6 | Made AI Task archive rows open their stored result detail | Commit pending; Personal PWA AI Task archive rows now behave like the other archive boards: tapping a row selects it, renders the archived result/memo draft from the existing list payload, provides `[ BACK ]` and `[ copy ]`, and shows `SAVED` instead of offering a duplicate save for already-applied tasks. Applied web-search tasks fall back to the preserved memo draft text when the completion receipt has replaced the result payload | Static PWA promotion pending; frontend syntax, focused AI Tasks tests, full family-portal tests (`65/65`), and H3 helper tests (`369 OK`) pass locally. UI/read-only archive behavior only; no backend route, source fetching, Brain, or write-policy change |
