...
| 2026-09-02 | 6 | Corrected personal portal default branding | `kaosgdd.net` startup/default HTML now identifies as `KaosGDD` instead of `Kaos Family`, including the page title, iOS web-app title, and initial visible fallback header; the `family.kaosgdd.net` host override still switches to `Kaos Family`; JavaScript syntax check, 35 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; HTML/test-only branding correction with no route/API behavior change |
