...
| 2026-09-03 | 7 | Improved HIRA medicine benefit discovery for AI Tasks | Commit `89dae70`; HIRA generic search was replaced with a dedicated 보험인정기준 POST-search adapter, because the old generic HIRA search URL returned dead/noisy results and medicine criteria may be indexed by ingredient or class instead of Korean brand name. Governor now expands examples such as `알모그란/알모트립탄 -> Almotriptan/편두통 치료제` and `글리아티린 -> Choline alfoscerate/콜린알포세레이트`, ranks newer HIRA criteria ahead of old product-specific rows, filters search/menu noise, and tells KaosBrain-OpenAI to add known ingredient/class alternate queries for Korean medicine brands. Validation found live HIRA candidates for `알모그란정 급여기준`, with 2024 `편두통 치료제` first and 2013 `Almotriptan 경구제 (품명: 알모그란정)` second; extracted HIRA source text includes `Almotriptan` and the 2024 criteria. Syntax checks, H3 Docker suite (`369 tests OK`), and Brain Docker suite (`353 tests OK`) pass | Deployment pending at the time of this log entry; expected production impact is search/source selection only. AI Tasks remain read-only until the user explicitly saves/copies elsewhere, and Governor still rejects non-allowlisted hosts |
