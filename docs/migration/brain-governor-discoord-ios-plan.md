...
| 2026-09-04 | 6 | Collapsed optional AI Task source fields under Details | Commit pending; Personal PWA AI Task composer now keeps `PROMPT` and `PDF` visible while optional `URL` and source `TEXT` fields live inside a default-collapsed inline `DETAILS` disclosure, using the same compact bracket-command visual language as the rest of the archive UI. Cache-busted static assets to `styles.css?v=299` and `app.js?v=297` | Static PWA promotion pending. Local validation passes: `node --check apps/family-portal/app.js`, focused AI Tasks UI tests, full family-portal tests (`66/66`), and `git diff --check`. UI-only trim; no backend route, source policy, Brain behavior, or write capability changed |
