...
| 2026-09-02 | 6 | Aligned desktop content top with left menu card | Main-profile desktop `.view` now has a specificity-matched media override so the earlier mobile/default `padding-top: 14px` no longer pushes every page content panel below the left navigation card; versioned `styles.css?v=282`; JavaScript syntax check, 40 frontend tests, and `git diff --check` pass | Deployment pending; CSS-only shell alignment fix for all main desktop pages with no route/API/data behavior change |
