...
| 2026-09-01 | 6 | Deployed first Documents upload workflow | Commit `5f82adb`; added a governed PDF upload path from the personal PWA to Paperless, backed by a small `/data/documents/intake.json` Inbox board with SHA duplicate protection; `#/add-document` is owned by Documents and the top `+` opens it; versioned `styles.css?v=267`, `documents.js?v=3`, and `app.js?v=247`; 19 frontend tests and 267 Governor tests pass; H3 `governor-api` is healthy and static files are byte-synced | Deployed to H3; no synthetic PDF uploaded to live Paperless during validation |
