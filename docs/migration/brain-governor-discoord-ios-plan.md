...
| 2026-09-02 | 6 | Cleaned main Settings into a read-only control room | Main `kaosgdd.net/#/settings` now renders as a compact read-only control room: overview, System Status, Governor summary, domain/source-of-truth map, and admin console links; removed the main Settings editor stack for Mail Organizer, generated calendar toggles, event presets, recurring tasks, and holiday sync while leaving family Settings and backend APIs untouched; Settings route now loads only system and Governor status on the main profile; versioned `styles.css?v=281` and `app.js?v=267`; JavaScript syntax check, 40 frontend tests, and `git diff --check` pass | Deployment pending; expected production impact is less clutter and fewer PWA write surfaces on main Settings, with no Governor/API/data migration |
