...
| 2026-09-04 | 6 | Enabled H4 OpenClaw/OAuth as the broad AI Tasks web-search backend | Pending commit in this turn; H4 OpenClaw config now enables `tools.web.search` with provider `codex`, `openaiCodex.mode=live`, and `tools.web.fetch` with readability. KaosBrain `/internal/ai-tasks/web/preview` still prefers a direct `KAOSBRAIN_OPENAI_API_KEY_FILE` when present, but now falls back to KaosBrain-OpenAI via OpenClaw/OAuth when no API key is configured, requiring structured JSON with title/content/sources for PWA rendering; PWA error text no longer implies API-key-only setup; static JS cache bumped to `app.js?v=302` | Pending deployment in this turn. Validation so far: `node --check apps/family-portal/app.js`, focused AI Tasks UI tests, full family-portal tests (`69/69`), local Brain web/OpenClaw tests (`31 OK`, `7 skipped`), Brain helper Docker suite (`356 OK`), and `git diff --check` pass. Production impact is read-only web-search capability through OpenClaw OAuth; no automatic Memos/Paperless/Radicale/mail/fax writes |
