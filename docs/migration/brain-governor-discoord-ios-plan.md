...
| 2026-09-03 | 7 | Routed AI Tasks web search through official health allowlist | Commit `9fa287a`; Personal AI Tasks `WEB SEARCH` now uses KaosBrain-OpenAI/OpenClaw to create a structured search job, then Governor searches/fetches only allowlisted Korean official/public health domains before sending source excerpts back to KaosBrain-OpenAI for summary/reasoning. Added H4 internal routes `/internal/ai-tasks/official-web/plan` and `/internal/ai-tasks/official-web/summarize`, H3 official-search adapters for MOHW/KDCA/MFDS/HIRA public search pages, broader source-host allowlist, PDF/text source fetching under the same host policy, clearer PWA error labels, and `app.js?v=294`; local syntax checks, focused Brain web-task tests, H3 Docker suite (`369 tests OK`), and Brain Docker suite (`353 tests OK`) pass | Not broad web browsing: Governor rejects arbitrary hosts and archives read-only results with checked date, plan, and sources. Search adapters initially cover `mohw.go.kr`, `kdca.go.kr`, `mfds.go.kr`, and `hira.or.kr`; other allowlisted national/affiliated health domains may appear as fetched source links but are not all searched directly yet. Deployment/smoke still pending for this commit |
