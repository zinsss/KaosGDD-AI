...
| 2026-09-02 | 6 | Anchored the desktop shell plus to the left card | Desktop main menu picker no longer acts as the positioning parent for the shell `+`, so the button anchors to the left card/header instead of drifting beside the active menu row; versioned `styles.css?v=278`; JavaScript syntax check, 35 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; CSS-only desktop visual fix with no action/API behavior change |
