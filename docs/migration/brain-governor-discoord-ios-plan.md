...
| 2026-09-04 | 7 | Added chart-note guidance to medicine benefit AI Task answers | Commit `pending`; KaosBrain-OpenAI official-web summaries now explicitly include a source-bounded `차트 기재 추천` section for Korean medicine insurance/benefit questions such as 급여기준, 요양급여, 본인부담, 투여조건, or 삭감. The section should provide copyable Korean chart-note examples with placeholders and a checklist of source-supported clinical facts to verify, while refusing chart-note guidance if the fetched criteria are insufficient. Focused Brain prompt tests pass and Brain Docker suite passes (`354 tests OK`) | H4 Brain deployment pending at the time of this log entry. This is answer-format guidance only: no EMR write, no PWA write expansion, and no change to Governor source fetching or HIRA/health.kr search |
