...
| 2026-09-01 | 6 | Reduced PWA bottom safe-area reserve | Main PWA pages now use a minimal `max(8px, safe-area-inset-bottom)` bottom reserve instead of 40 px; embedded Memos image override uses the same minimal bottom reserve; versioned `styles.css?v=257` and `kaosgdd-memos-web:0.1.7-minimal-safe-area` | Presentation-only follow-up; promotion pending |
