...
| 2026-09-01 | 5 | Added Naver lifecycle polling for INBOX | Changed the Naver mail lifecycle default and H3 runtime configuration to `MAIL_NAVER_FOLDERS=INBOX,세무사,영덕군보건소`; existing INBOX mail is expected to baseline on first scan because `MAIL_NAVER_MARK_EXISTING_ON_FIRST_RUN=true`, then future new INBOX mail can trigger the same worker-owned minimal mail notification path | Implementation/testing in progress |
