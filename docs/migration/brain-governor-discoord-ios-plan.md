...
| 2026-09-04 | 7 | Fixed AI Tasks treatment answers that only saw PubMed titles | Implementation commit pending; diagnosed the unhelpful BPPV screenshot as PubMed fetches returning a `Cookies must be enabled` page, leaving KaosBrain with titles/URLs instead of abstracts. Governor now fetches PubMed article metadata/abstracts through NCBI E-utilities from the PMID, skips PubMed records without usable abstracts instead of forwarding cookie/search chrome, upgrades treatment-option prompts from accidental `summary` plans to `treatment_options`, adds BPPV/이석증 aliases, diversifies source candidate collection across first/alternate queries, and ranks practical guideline/review records above adherence/barrier/navigation noise. Added AAO-HNS/ENTNET to the treatment-source allowlist/adapters | Validation passes: Python syntax checks, frontend tests (`78/78`), `git diff --check`, H3 helper tests (`369 OK`), and Brain helper tests (`358 OK`). Live BPPV probe now returns `task=treatment_options` and PubMed abstract text with the AAO-HNS BPPV clinical practice guideline record ranked first. Deployment pending in this turn |
