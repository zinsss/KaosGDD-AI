...
| 2026-09-02 | 6 | Fixed desktop archive column alignment | Desktop archive rows now use wider fixed lanes for `NO.` and `DATE`, keep Memos/long generated IDs clipped with ellipsis instead of bleeding into the date, and preserve one-line titles; versioned `styles.css?v=275`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; CSS-only desktop visual polish with no data/API behavior change |
