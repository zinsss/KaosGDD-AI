...
| 2026-09-02 | 6 | Cleaned the main Settings page layout | Main `kaosgdd.net` Settings now renders as a compact Archive Terminal-style settings board with KaosGDD overview, System Status, a compact Governor summary, and grouped automation/template sections; Family Settings remains on the family panel layout; versioned `styles.css?v=279` and `app.js?v=264`; JavaScript syntax check, 36 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; layout/organization cleanup only, with no route/API/write-policy behavior change |
