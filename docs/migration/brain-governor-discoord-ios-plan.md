...
| 2026-09-04 | 7 | Merged AI Tasks type tabs into one prompt/source composer | Commit pending; Personal PWA AI Tasks now has one `AI TASK` composer instead of separate `WEB SEARCH` and `DOC -> MEMO` tabs. Prompt-only submissions use the official-source web-search path, while adding a PDF, URL, or pasted text uses the existing source/document memo-draft path. The backend routes remain separate behind the UI, and result cards still require explicit `SAVE MEMO` before writing to Memos | Static PWA promotion pending; targeted frontend syntax/UI checks pass locally. This is a UI routing simplification only: no new write capability, no broader source allowlist, and no change to H4 Brain behavior |
