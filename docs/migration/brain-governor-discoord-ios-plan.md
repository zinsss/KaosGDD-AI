...
| 2026-09-02 | 6 | Reduced the shell plus circle size | The page-aware shell `+` keeps its glyph style and behavior but uses a smaller 36 px circular visual on mobile and desktop, with adjusted desktop header spacing; versioned `styles.css?v=277`; JavaScript syntax check, 34 frontend tests, deploy-helper syntax check, compose validation, and `git diff --check` pass | Deployment pending; CSS-only visual polish with no action/API behavior change |
