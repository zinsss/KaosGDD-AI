...
test("the navigation contract loads before the portal application", () => {
  ...
  const styleIndex = index.indexOf('href="/styles.css?v=357"');
  ...
  const applicationIndex = index.indexOf('src="/app.js?v=352"');
...
test("family font settings include the custom web fonts", () => {
  ...
  assert.match(appSource, /FAMILY_FONT_OPTIONS = new Set\(\[[^\]]*"watermelon"[^\]]*"kidari-balloon"[^\]]*"halfmoon"/);
  assert.match(appSource, /<option value="watermelon"/);
  assert.match(appSource, /<option value="kidari-balloon"/);
  assert.match(appSource, /<option value="halfmoon"/);
  assert.match(styles, /font-family: "Watermelon";[\s\S]*EF_watermelonSalad\.woff2/);
  assert.match(styles, /font-family: "SchoolSafetyKidariBalloon";[\s\S]*HakgyoansimKidaripungseonL\.woff2/);
  assert.match(styles, /font-family: "SchoolSafetyHalfMoon";[\s\S]*HakgyoansimBandalL\.woff2/);
  assert.match(styles, /data-family-font="watermelon"[\s\S]*"Watermelon"/);
  assert.match(styles, /data-family-font="kidari-balloon"[\s\S]*"SchoolSafetyKidariBalloon"/);
  assert.match(styles, /data-family-font="halfmoon"[\s\S]*"SchoolSafetyHalfMoon"/);
  assert.match(translations, /"settings\.fontWatermelon": "Watermelon"/);
  assert.match(translations, /"settings\.fontKidariBalloon": "학교안심 키다리풍선"/);
  assert.match(translations, /"settings\.fontHalfMoon": "학교안심 반달"/);
});
...
test("calendar month panel rendering is delegated to the view module", () => {
  ...
  assert.ok(index.indexOf('src="/calendar-view.js?v=2"') < index.indexOf('src="/app.js?v=352"'));
