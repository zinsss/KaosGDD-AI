...
test("Rouny detail offers Make default and identifies the current default", () => {
  ...
  assert.match(indexSource, /src="\/translations\.js\?v=193"/);
  assert.match(indexSource, /src="\/app\.js\?v=360"/);
});
...
test("Rouny timeline uses slightly taller hourly cells", () => {
  ...
  assert.match(indexSource, /href="\/styles\.css\?v=367"/);
});
...
test("Rouny uses one centered line on screen and compacts printed class labels", () => {
  ...
  assert.match(
    styles,
    /\.rounyBlockHandle \{[\s\S]*?top: 2px;[\s\S]*?left: -7px;[\s\S]*?border-radius: 50%;/,
  );
  assert.doesNotMatch(styles, /\.rounyTimelineDayColumn:last-child \.rounyBlockHandle/);
  ...
  assert.match(styles, /@media print \{[\s\S]*?\.rounyBlockHandle \{\s*display: none !important;/);
  assert.match(styles, /@media print \{[\s\S]*?\.rounyBlock strong \{[\s\S]*?white-space: nowrap;/);
});

test("Rouny timetable and class cards keep square corners on screen and print", () => {
  assert.match(styles, /\.rounyTimelineGrid \{[\s\S]*?border-radius: 0;/);
  assert.match(styles, /\.rounyBlock \{[\s\S]*?border-radius: 0;/);
  assert.match(styles, /@media print \{[\s\S]*?\.rounyTimelineGrid \{[\s\S]*?border-radius: 0;/);
  assert.match(styles, /@media print \{[\s\S]*?\.rounyBlock \{[\s\S]*?border-radius: 0;/);
});
