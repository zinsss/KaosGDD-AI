...
test("Scribble is a small staging inbox with both handoff actions", () => {
  ...
  assert.match(index, /src="\/navigation\.js\?v=9"/);
  assert.match(index, /src="\/scribble\.js\?v=1"/);
  assert.match(index, /href="\/styles\.css\?v=362"/);
  assert.match(index, /src="\/scribble-view\.js\?v=4"/);
  assert.ok(index.indexOf('src="/scribble-view.js?v=4"') < index.indexOf('src="/app.js?v=356"'));
