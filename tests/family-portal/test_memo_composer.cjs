...
test("memo composer uploads arbitrary files and links them to a private memo", () => {
  ...
  assert.match(stylesSource, /\.memoFileControl \{[\s\S]*font-family: "Sarasa Gothic Mono"[\s\S]*!important;/);
...
test("main and family memos routes render native archive board controls", () => {
  ...
  assert.match(stylesSource, /\[data-archive-kind="memos"\] \.memoToolbarPanel \.archiveTagChip \{\s*border-color: rgba\(76, 86, 106, 0\.72\);\s*background: rgba\(20, 24, 31, 0\.72\);/);
