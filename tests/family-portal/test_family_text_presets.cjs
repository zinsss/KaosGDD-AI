...
test("family preset text assets include styles, translations, and cache-busted bundles", () => {
  ...
  assert.match(indexSource, /href="\/styles\.css\?v=320"/);
  assert.match(indexSource, /src="\/translations\.js\?v=183"/);
  assert.match(indexSource, /src="\/text-presets\.js\?v=1"/);
  assert.match(indexSource, /src="\/app\.js\?v=328"/);
  assert.ok(indexSource.indexOf('src="/text-presets.js?v=1"') < indexSource.indexOf('src="/app.js?v=328"'));
});
