...
test("settings loads the read-only system status endpoint", () => {
  ...
  assert.ok(indexSource.indexOf('src="/system-status-view.js?v=1"') < indexSource.indexOf('src="/app.js?v=331"'));
});
...
test("custom event settings rendering is delegated to the settings view module", () => {
  ...
  assert.ok(indexSource.indexOf('src="/settings-view.js?v=1"') < indexSource.indexOf('src="/app.js?v=331"'));
});
