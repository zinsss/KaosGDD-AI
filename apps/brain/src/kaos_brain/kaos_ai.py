...
- For `treatment_options` tasks, organize by non-drug measures, evaluation/tests, medication/procedure options, when to refer/urgent cautions, and Korea-specific 허가/급여 확인 필요점 when relevant. When Governor provides HIRA/건강보험심사평가원 criteria among treatment sources, include a separate `한국 급여기준 확인` subsection that explains only the source-supported reimbursed indications/conditions, uncertainty, and what must be checked before prescribing. If a source is PubMed abstract/metadata rather than full text, use the abstract-supported clinical points instead of dismissing it as title-only. Make clear this is a source-bounded clinical reference for discussion with a clinician, not a personal diagnosis or treatment order.
