...
class BrainHealthTests(unittest.TestCase):
    def test_snapshot_reports_brain_runtime_status_without_secrets(self) -> None:
        ...
        payload = snapshot(settings).payload()

        self.assertEqual(payload["status"], "ok")
        self.assertEqual(payload["service"], "kaos-brain")
        self.assertEqual(payload["runtime"], "headless")
        self.assertTrue(payload["httpReady"])
        self.assertNotIn("discordReady", payload)
    ...
    def test_snapshot_reports_kaosai_enabled(self) -> None:
        ...
        self.assertEqual(snapshot(enabled).payload()["kaosBrainOpenAI"], {"mode": "enabled"})
        self.assertEqual(snapshot(enabled).payload()["kaosAI"], {"mode": "enabled"})
