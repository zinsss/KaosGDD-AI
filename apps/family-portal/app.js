...
const ROUNY_TIMELINE_HOUR_HEIGHT = 72;
...
const state = {
  ...
  addTaskDraft: null,
  smartEventInput: "",
  smartEventAiProposals: null,
  smartEventAiLoading: false,
  smartEventAiError: "",
  smartEventPreviewSource: "grammar",
...
function aiTaskErrorMessage(code) {
  ...
  const messages = {
    ...
    ai_task_official_web_brain_not_configured: "KaosBrain official-search endpoints are not configured.",
    ai_task_official_web_brain_request_failed: "Could not reach KaosBrain official-search pipeline.",
    ai_task_official_web_brain_invalid_json: "KaosBrain returned an unreadable official-search response.",
    ai_task_official_web_missing_plan: "KaosBrain did not return a search plan.",
    ai_task_official_web_missing_result: "KaosBrain did not return a summary.",
    ai_task_official_web_query_required: "KaosBrain did not produce a searchable query.",
    official_web_sources_required: "Governor did not provide sources for KaosBrain to summarize.",
    ai_task_official_web_sources_not_found: "Governor could not find readable official sources in the allowlist.",
    ai_task_official_web_invalid_result: "KaosBrain returned an incomplete official-search result.",
    ...
    kaosbrain_web_search_not_configured: "KaosBrain web search is not configured.",
    kaosbrain_web_search_unavailable: "KaosBrain OpenClaw web search is unavailable.",
    ...
    ai_task_source_host_not_allowed: "That source is outside the official health allowlist.",
    ai_task_source_fetch_failed: "Could not fetch the source URL. Try a specific article page or paste the source text.",
    ai_task_source_unsupported_content_type: "That source type is not readable yet.",
    ai_task_source_too_large: "That source is too large for AI Tasks.",
...
function aiTaskViewContext() {
  return {
    ...
    archiveStatusClass,
    archiveMeta,
    aiTaskMemoContentFromPreview,
    aiTaskIsResultPreview,
    aiTaskIsOfficialWebPreview,
    aiTaskErrorMessage,
  };
}

function aiTaskSourceHost(url) {
  return window.KAOS_AI_TASKS_VIEW.aiTaskSourceHost(url);
}

function aiTaskSourceQualityKey(source) {
  return window.KAOS_AI_TASKS_VIEW.aiTaskSourceQualityKey(source);
}

function aiTaskSourceQualityCounts(sources, textbookSources) {
  return window.KAOS_AI_TASKS_VIEW.aiTaskSourceQualityCounts(sources, textbookSources);
}

function renderAiTaskSourceQuality(sources, textbookSources, labels = {}) {
  return window.KAOS_AI_TASKS_VIEW.renderAiTaskSourceQuality(aiTaskViewContext(), sources, textbookSources, labels);
}

function renderAiTaskPlan(plan) {
  return window.KAOS_AI_TASKS_VIEW.renderAiTaskPlan(aiTaskViewContext(), plan);
}

function renderAiTaskSources(sources) {
  return window.KAOS_AI_TASKS_VIEW.renderAiTaskSources(aiTaskViewContext(), sources);
}

function renderAiTaskTextbookSources(sources) {
  return window.KAOS_AI_TASKS_VIEW.renderAiTaskTextbookSources(aiTaskViewContext(), sources);
}

function renderAiTaskStatePanel(preview, labels = null) {
  return window.KAOS_AI_TASKS_VIEW.renderAiTaskStatePanel(aiTaskViewContext(), preview, labels);
}
...
function renderFax() {
  return window.KAOS_FAX_VIEW.renderFax(faxViewContext());
}

function faxViewContext() {
  return {
    state,
    escapeHtml,
    archiveMeta,
...
function renderMail() {
  return window.KAOS_MAIL_VIEW.renderMail(mailViewContext());
}

function mailViewContext() {
  return {
    state,
    escapeHtml,
    archiveDateParts,
...
function renderAiTasks() {
  return window.KAOS_AI_TASKS_VIEW.renderAiTasks(aiTaskViewContext());
}

function renderMemos() {
  return window.KAOS_MEMOS_VIEW.renderMemos(memosViewContext());
}
...
function renderSystemStatusPanel() {
  return window.KAOS_SYSTEM_STATUS_VIEW.renderSystemStatusPanel(systemStatusViewContext());
}

function systemStatusViewContext() {
  return {
    state,
    uiText,
    escapeHtml,
    ...
    recurringWorkerSummary,
    renderSystemServiceRows,
    recurringWorkerStatusLine,
    runtimeStatusLine,
  };
}
...
function renderCustomEventSettings() {
  return window.KAOS_SETTINGS_VIEW.renderCustomEventSettings(settingsViewContext());
}

function settingsViewContext() {
  return {
    state,
    uiText,
    escapeHtml,
    ...
    defaultTaskDueTime: DEFAULT_TASK_DUE_TIME,
    taskMemoPlaceholder: TASK_MEMO_PLACEHOLDER,
    recurringFrequencyLabel,
    recurringCreationPolicyLabel,
    recurringOwnerLabel,
  };
}
...
function renderRecurringTaskSettings() {
  return window.KAOS_SETTINGS_VIEW.renderRecurringTaskSettings(settingsViewContext());
}
...
document.addEventListener("click", async (event) => {
  ...
  if (await window.KAOS_TEXT_PRESETS.handleClick(textPresetContext(), event)) {
    return;
  }
