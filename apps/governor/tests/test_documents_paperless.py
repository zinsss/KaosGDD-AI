    ...
    def test_get_document_uses_paperless_detail_endpoint(self) -> None:
        urlopen = mock.Mock(
            side_effect=[
                ...
                FakeResponse(body=b'{"results":[{"id":7,"name":"medical"},{"id":8,"name":"tax"}]}'),
            ]
        )
        service = PaperlessDocumentService(self.config(), urlopen=urlopen)
        ...
        self.assertEqual(document.tag_names, ("medical", "tax"))
        self.assertEqual(urlopen.call_args_list[1].args[0].get_method(), "GET")
        self.assertIn("/api/tags/?", urlopen.call_args_list[1].args[0].full_url)

    def test_document_detail_keeps_ids_when_tag_lookup_fails(self) -> None:
        urlopen = mock.Mock(
            side_effect=[
                FakeResponse(
                    body=(
        ...
        self.assertEqual(document.tag_ids, (7, 8))
        self.assertEqual(document.tag_names, ())
