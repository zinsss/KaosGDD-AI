...
class TimezoneTests(unittest.TestCase):
    ...
    def test_kaos_today_ends_with_unlabeled_plain_text_inspiration(self) -> None:
        payload = shortcut_briefing_payload(
            ...
            bible_reference="시편 119:105",
            bible_text="주의 말씀은 내 발에 등이요 내 길에 빛이니이다",
            quote_text="If you correct your mind, the rest of your life will fall into place.",
            quote_author="Laozi",
        )
...
class BrainToolServerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_task_create_proposal_requires_confirmation_before_write(self) -> None:
        ...
        payload = await response.json()
        self.assertTrue(payload["confirmationId"].startswith("conf_"))
        self.assertEqual(payload["task"]["profile"], "main")
    ...
    async def test_task_create_allows_personal_task_without_due_date(self) -> None:
        ...
        self.assertEqual(proposal.status, 201)
        proposal_payload = await proposal.json()
        self.assertEqual(proposal_payload["task"]["profile"], "main")
    ...
    async def test_task_create_approval_preserves_collection_id(self) -> None:
        ...
        self.assertEqual(proposal.status, 201)
        proposal_payload = await proposal.json()
        self.assertEqual(proposal_payload["task"]["profile"], "supplies")
